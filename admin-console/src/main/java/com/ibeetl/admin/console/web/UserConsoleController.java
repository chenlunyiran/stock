...
public class UserConsoleController {
	...
	@GetMapping(MODEL + "/changePasswordSelf.do")
	public ModelAndView changePasswordSelf() {
		Long id = platformService.getCurrentUser().getId();
		CoreUser user = userConsoleService.queryById(id);
		ModelAndView view = new ModelAndView("/admin/user/changePasswordSelf.html");
		view.addObject("user", user);
		return view;
	}
	...
	@Function("user.reset")
	@ResponseBody
	public JsonResult changePassword(Long id, String password, String password2) {

		int re = userConsoleService.resetPassword(id, password.replace(" ", ""),password2.replace(" ", ""));
		if(re==-2){
			return new JsonResult().failMessage("密码不能为空格且两次密码输入需相同");
		}
	 ...
	 * @return
	 */
	@PostMapping(MODEL + "/changePasswordSelf.json")
	@ResponseBody
	public JsonResult changePasswordSelf(String password, String password2) {
		Long id = platformService.getCurrentUser().getId();
		int re = userConsoleService.resetPassword(id, password.replace(" ", ""),password2.replace(" ", ""));
		if(re==-2){
			return new JsonResult().failMessage("密码不能为空格且两次密码输入需相同");
